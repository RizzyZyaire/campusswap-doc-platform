...
| **D17** | **只能一篇一篇导入**（用户：「想支持一次批量导入多个 .md，上传以后支持直接导入或是再编辑如何呢」） | 单篇导入是我按"最小可用"做的，但现实场景是"一批公文进来"：逐个选文件、每篇都要等一次跳转，很费手 | ① `ImportMarkdownButton` 加 `multiple`（一次 ≤50 个，逐类汇总跳过原因）；② 新增 `MarkdownImportDialog.vue`：文件表（勾选/改标题/字数/逐条结果与「重试失败的」）+ 统一分类/标签 + 两个落地按钮；③ 新增 `stores/importQueue.ts`（`sessionStorage`），编辑器加批次横幅，**保存后自动跳下一篇** | 端到端实测（`import-test.mjs`，用 CDP `DOM.setFileInputFiles` 塞 3 个真文件）：弹窗 3 行、标题自动取 H1/文件名 → 「建草稿并逐篇编辑」→ 建 3 篇、跳到第一篇、横幅「还剩 2 篇（当前第 1 篇）」→ 点保存 → **自动跳到第 2 篇、横幅变「还剩 1 篇」**；跑完已重灌演示库 |
