...
| 控件 | 位置 | 点击后 |
|---|---|---|
| 通知铃（带未读红点） | 产品顶栏右侧、身份框左侧 | 弹出 **320px 通知面板**：标题栏「通知 · N 条未读」+「全部标为已读」；正文逐条 = 类别徽标（维护/制度/流程/…）+ 标题 + 时间，未读行带红点；读完红点消失、空态显示「已读完了」 |
...
- 身份菜单里的「切换账号」在本学期课程作业里落地为：**退出登录 → 登录页换账号登录**（预览稿为了演示方便做成即时切换身份）。
