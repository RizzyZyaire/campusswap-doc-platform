...
> 视觉与文案以 `docs/02-design/UI-PREVIEW.html`（**v5 预览稿**，配套自检 `ui-preview.smoke.mjs` 161 项）为准，本表定的是路由、权限与行为。
...
| 主题横幅 ×6 | `hero-*.jpg` | 同上目录 6 张（见下表） | 各 760×289 JPEG q82（≈2.63:1，与显示槽位同比例） | 工作台横幅右侧照片，**一套主题一张，全站不复用** |
...
| 主题 | 照片 | 源图 | 说明 |
|---|---|---|---|
| `hebtu` 师大蓝 | 时光塔 | 时光塔.jpg（1000×533） | 校园地标＋蓝天，呼应蓝调 |
| `gingko` 银杏暖 | 银杏大道 | 校内行道.jpg（1000×666） | 秋日银杏，呼应主题名 |
| `celadon` 青瓷 | 玉兰与蓝天 | 多花.jpg（2000×1333） | 玉兰＋深蓝天，清爽 |
| `ink` 墨玉青 | 时光塔下 | 时光塔下.jpg（1000×750） | 塔＋绿树，青绿感 |
| `jiang` 师大绛 | 东门黎明 | 东门黎明.jpg（1000×465） | 黎明金红，呼应绛红 |
| `night` 墨夜 | 教学楼夜景 | 教学楼夜景.jpg（1000×563） | 夜景航拍，呼应深色 |

> **裁剪纪律（v4 教训）**：横幅照片一律按**显示比例**裁剪后再缩放，缩放比恒 < 1（显示宽约 520px，源图 760px）；禁止把宽幅全景拉伸铺满。所有照片在文档里只内联一次，通过 `--photo-*` CSS 变量被横幅与画廊分别引用（自检断言：jpeg data URI 恰好 8 处）。
...
4. **然后**才开 M5 前端编码；前端按本规格 §1~§9 施工，视觉以 `UI-PREVIEW.html`（**v5**）为准。
...
| 5 | 全量回归：9 个产品机检 **504** 项（m0 13 / m1 15 / api-spec 24 / m2 17 / db-deep 9 / m3 36 / m3-http 142 / m4 30 / m4-http 218）+ 预览稿自检 **161** 项 = **665 项** | 全部 0 失败（2026-09-23 实测） |
| 6 | 本规格 §1~§9 与 `UI-PREVIEW.html`（**v5**）逐页对账 | 人工核对：路由、权限、四态、主题令牌 |
