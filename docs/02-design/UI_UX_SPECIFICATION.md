...
| 版本 | **v2.7**（v2.6 之上补**批量导入 Markdown**：多选文件 → 预览弹窗（逐篇改标题、统一分类/标签）→ 「直接建草稿」或「建草稿并逐篇编辑」（编辑器带批次横幅、保存后自动跳下一篇）；v2.6 = 第二轮上手反馈：登录页/顶栏/横幅已修 + 新增「上传 Markdown」与「关于平台」；v2.5 = 第一轮四处返工：顶栏辨识度、主题选择器独立占最右、登录页可换主题+塔图+右栏、工作台横幅按钮点击被装饰层挡住；v2.4 = M5 收口的**三处订正**：§8.5 布局的「发文单位→拟稿人」「可见范围改只读文本，并补积分标记」、§8.8 与 §10.6 的口径统一（下架无接口 ⇒ 无入口，预览稿同步删掉那个按钮）、§8.7「审核意见必填」按后端 `@NotBlank` 订正；v2.3 内容：§8.5 编辑页 **409 的两种处置**——状态冲突切只读 / 版本冲突不切只读并给"刷新内容"；v2.2 内容：顶栏弹层、行级悬停、A 版式与时光塔首页、主题三字名） |
| 日期 | 2026-09-23 |
| 状态 | **Frozen（已冻结）** |
...
## 0.4 变更记录（v2.2 → v2.3，2026-09-23 晚 · 课件 4.1/5.1 对齐）

| # | 变更 | 原因 | 落位 |
|---|---|---|---|
| 1 | **编辑页 409 拆成两种处置**：① 状态冲突（归档/回收站只读）→ 切只读模式；② **版本冲突**（`versionNum` 与库中不一致）→ **不切只读**，顶部横幅 + 轻提示「该文档已被他人修改（当前版本 v3），请刷新后重试」+「刷新内容」按钮 | 后端 `PUT /api/documents/{id}` 新增必填 `versionNum`，版本不一致返回 409（陈旧表单防覆盖，`ARCHITECTURE §17 ADR-07`）；两种 409 的处置完全不同，混在一起会让用户以为文档变只读了 | §8.5 |
| 2 | 编辑页关键交互补一条：**保存必须回传 `versionNum`**（打开编辑页时从详情接口记下的那一版），收到版本冲突 409 后按上面的横幅流程处理 | 同上；M5 未开工，现在定契约最便宜 | §8.5、`MASTER-PLAN T5.6` |
| 3 | 预览稿升到 **v8.2**（配套自检 209 → **211** 项）：设计系统页补 409 版本冲突提示与 toast，「与前端的落地接口对照」登记该契约（三项变更 → 四项） | 预览稿严格照正式版（用户长期要求） | `UI-PREVIEW.html`、`ui-preview.smoke.mjs` |
| 4 | **预览稿 v8.3 = M5 收口对账**（自检项数不变，仍 211）：① 删掉内容治理里的「下架」按钮（`doc:offline` 无接口 ⇒ 无入口，§10.6）；② 编辑页「可见范围」由三选一下拉改为**只读文本**（库里没有分级可见字段）；③ 编辑页字段名与只读态写清楚（拟稿人，非"发文单位"）。规格侧同步订正 §8.5 / §8.7 / §8.8 三处 | 这三处都是"预览稿有、后端不认"的差异；M5 落地时必须二选一，选择**照后端**并回头改预览稿，避免留下点了没反应的控件 | §8.5、§8.7、§8.8、§10.6 |
| 7 | **v2.7 = 批量导入 Markdown（2026-09-24）**：`ImportMarkdownButton` 支持 `multiple`；新增 `MarkdownImportDialog.vue`（文件表：勾选/改标题/字数/逐条结果与重试；统一分类 + 统一标签；两个落地按钮）与 `stores/importQueue.ts`（待编辑队列，`sessionStorage` 持久化）；编辑器加批次横幅（第 N / M 篇 + 跳过这篇 + 结束批次），**保存后自动跳下一篇** | 用户问"上传以后支持直接导入或是再编辑如何呢"——两种都要：一批公文通常同分类，先批量落库最省事；但标题与正文仍需逐篇过一眼，所以给"逐篇编辑"队列并把进度显式化（不然用户不知道还剩几篇） | §8.6、`M5-CLOSURE §3 D17` || 6 | **v2.6 = 第二轮上手反馈（2026-09-24）**：① 顶栏右侧控件组原本没贴右（`top-right` 的 `margin-left:auto` 没用上）→ 主题选择器补到真·右端；② **加「直达」下拉**（预览稿预览条上那个直达的正式版，只列当前身份有权限去的地方）；③ **顶栏底色跟着主题主色走**（用户："银杏暖/墨夜黑以外还是白的"）→ `--primary` 18%→7% 渐变 + 主色 46% 分隔线，六套主题各一色；④ **左上角品牌区改成按钮**，点开「关于平台」（84px 校徽原图 + 7 条平台事实 + 素材来源说明），并 `user-select:none` 消掉文本插入符（用户："单击会出现打字时的竖虚线"）；⑤ **新增「上传 Markdown」**：编辑器顶部「导入文件」把本地 .md/.markdown/.txt 读进表单，「我的文档」页「上传 Markdown」直接建草稿（只收纯文本，Word/PDF 明确提示先另存 —— PRD 非目标 O7） | ①③⑤ 是"我按预览稿实现但没在真实窗口里核对"的产物；②④ 是用户对预览稿既有细节的追认。**M6 走查清单加两条：控件是否真的贴到它该在的边；每个可点/不可点的区域都要有明确反馈** | §8.1、§8.2、§8.6、§8.15、`M5-CLOSURE §3 D15/D16` || 5 | **v2.5 = 用户上手实测后的四处返工**（2026-09-24）：① 顶栏**要有辨识度**（原来与页面底几乎同色）—— 改主题色淡染玻璃 + 2px 分隔线 + 轻投影；② **主题选择器独立占顶栏最右**（原来夹在通知与身份之间），并加一道细分隔线；③ 登录页补主题选择器、塔图放大看见塔尖、右栏加主题底色与演示账号；④ 工作台横幅三个按钮**点不动**（装饰光晕挡指针，见 §8.2 根因） | 这四条都是"跑起来给人看"才暴露的：静态机检全绿、预览稿对照也一致，但**真实指针路径**与**窄/矮窗口**下的表现没人验过 → M6 走查清单要加"指针命中测试"与"两种窗口比例各截一张" | §8.1、§8.2、§8.15、`M5-CLOSURE §3 D11~D14` |
...
>
> **v2.5（用户实测反馈后落地，2026-09-24）三处**：
> ① **登录页也要能换主题** —— 登录页没有产品顶栏，而主题是 localStorage 持久化的，原来只能被动继承上次主题、想换也换不了；
>    现在右栏右上角常驻**主题选择器**（与顶栏同一个 `ThemePicker` 组件：三色点 + 中文名 + ▾ + 6 张色卡）。
> ② **塔图放大且必须看得见塔尖** —— `.login-aside .bg` 改 `background-size:auto 138%`、`background-position:50% 14%`；
>    原来的 `cover` 在"宽而扁"的窗口下会把塔尖裁掉（用户："至少能看见塔尖吧"）。
> ③ **右栏不再"纯白里孤零零一张表单"** —— 加主题色淡染渐变底 + 校徽「统一身份认证」抬头 + **演示账号一键填充**（三个内置账号，点一下填工号与口令）
>    + 字段间距（密码行与登录按钮之间原来完全没有外边距）。
...
- **v2.5 修订（用户实测"三个按钮全无悬停效果与点击跳转"）**：根因是**横幅上那层淡光晕挡住了指针** ——
  `.hero .lead:after`（radial-gradient）是 `.lead` 的最后一个子元素，而 `.hero .lead>*{position:relative}` 没给 `z-index`，
  于是光晕盖在按钮之上（实测：三个按钮的 `elementFromPoint` 全返回 `DIV.lead`），悬停样式不触发、真实点击也落不到链接上。
  修法（`main.css` 覆盖，生成物不手改）：`.hero .lead>*{position:relative;z-index:1}` + `.hero .lead:after{pointer-events:none}`；
  复验：`elementFromPoint` 返回按钮自身、悬停底色变成 `rgba(255,255,255,.28)`、点击后路由跳到 `/docs`。
  **推论给其它页面**：给容器加装饰性 `::after/::before` 覆盖层时，必须同时给它 `pointer-events:none` 或把内容抬到 `z-index:1`。
...
- **布局**：标题（大号无边框输入）→ 左栏编辑器（工具栏 + 源文，可切「编辑 / 预览 / 双栏」）右栏发布设置（分类·拟稿人·标签·摘要·积分标记·可见范围）
  - **v2.4 订正**：数据模型里**没有「发文单位」列**（PRD §2.3 已注明），预览稿 v2 起就用「拟稿人（自动取登录态，只读）」代替，本行此前漏改，现对齐；「可见范围」在库里也没有分级字段，界面按**只读文本**展示当前唯一行为（草稿仅本人可见 / 发布后全校可见），不再给三选一的下拉（那三个选项后端一个都不认）；「积分标记」是 `price_cents` 的入口（PRD §8 O3：不做真实支付，仅免费/积分标记）
- **四态**：空＝新建模式右栏预览「左侧输入 Markdown，这里实时预览」；加载＝编辑模式整页骨架 + 顶部细进度条，骨架期禁用输入；错误＝403 整页「你没有编辑这篇文档的权限」、400 字段级红字 + 自动滚动聚焦、**409 分两种**：① 状态冲突（归档 / 回收站只读）切只读模式「该文档当前状态不允许编辑」；② **版本冲突**（`versionNum` 与库中不一致）**不切只读**，顶部横幅 + 轻提示「该文档已被他人修改（当前版本 v3），请刷新后重试」，横幅上给「刷新内容」按钮（重新拉详情并覆盖本地表单，用户重新确认后再保存）、图片上传失败在源文原地留「点击重试」占位符；无权限＝缺 `doc:upload` 时工具栏插图按钮不渲染且粘贴/拖拽被拦截（纯文本不受影响）、缺 `doc:publish` 只保留「保存草稿」
- **v2.6 新增入口**：页头「导入文件」（编辑器顶部的 `ImportMarkdownButton`）把本地 `.md` / `.markdown` / `.txt` 读进正文；正文非空时先二次确认再覆盖；文件名当标题、正文首个 H1 与标题不同会给一句提示
- **关键交互**：未保存离开二次确认；有未完成的上传占位符时阻止保存并提示；**保存时必须回传 `versionNum`**（打开编辑页时从详情接口记下的那一版），收到版本冲突 409 后按上面的横幅流程处理 —— 2026-09-23 新增，见 `ARCHITECTURE §17 ADR-07` 与 `API_SPECIFICATION §9.5`
...
- **关键交互**：删除/彻底删除二次确认（彻底删除需**输入标题**确认，BR-08）；恢复失败 409 提示后刷新
  - **v2.6/v2.7 入口**：页头「上传 Markdown」**支持多选**——选本地 `.md` / `.markdown` / `.txt`（单文件 ≤512 KB、一次 ≤50 个），浏览器里读完打开**批量导入弹窗**：逐篇可改标题/取消勾选、统一选分类与标签，然后二选一 —— **「直接建 N 篇草稿」**（留在列表刷新）或 **「建草稿并逐篇编辑」**（把新文档放进待编辑队列，跳到第一篇，编辑器顶部显示「还剩 N 篇（当前第 M 篇）」，**保存后自动跳下一篇**，也可「跳过这篇」/「结束批次」）。逐条结果都在表里（成功/失败原因），失败可「重试失败的」。**只收纯文本**：Word/PDF 会明确提示"请先另存为 Markdown"（PRD 非目标 O7：不做 Office 在线预览与格式转换），超过 512 KB 也直接说明，不静默失败
...
- **关键交互**：驳回理由必填 1–255 字（BR-12），提交后写入 `rejectReason` 回传属主详情页
  - **v2.4 订正**：**「审核意见」也是必填**（1–255 字）—— 后端 `DocumentAuditDtoReq.remark` 上是 `@NotBlank`，与"通过时可写意见"的旧写法冲突；M5 按后端实现（必填），本行据此订正。三张统计里「本周已通过 / 已驳回」后端没有聚合接口，正式版口径 = **本页待审文档近 7 天的 AUDIT / REJECT 版本留痕计数**，卡片副文案写明该口径（不假装是全平台周报）
...
- **关键交互**：归档写入审核意见；恢复上架同时清空 `rejectReason`（T7）；彻底删除需输入标题确认
  - **v2.4 订正**：① 这里**没有「下架」按钮**（`doc:offline` 无接口 ⇒ 无入口，§10.6）—— 预览稿 v8.2 里那个按钮已在 v8.3 删除；② 概览卡数字来自**各状态一次 `pageSize=1` 查询取 `total`**（后端无聚合接口）；③ **拟稿人筛选的候选来自全量文档里出现过的作者**（用户目录 `GET /api/users` 要 `sys:user`，文档管理员没有该权限点），界面上写明这一点，不假装是通讯录；④ 彻底删除只能对回收站里的文档做，所以治理页保留「移入回收站 → 彻底删除」两步
...
| 身份框（头像 + 姓名 + 单位） | 产品顶栏（主题选择器左侧） | 弹出 **264px 身份菜单**：`身份介绍`（姓名 / 职称 / 角色 / 单位 / 有效权限 X/39）→「我的资料」（跳 `/me`）→「切换账号」→「退出登录」（回 `/login`）。 |
| **主题选择器（v2.5 定稿位置）** | **产品顶栏最右**，与「通知 / 身份」之间有一道细分隔线 | 弹出 **452px 色卡面板**（2 列 × 3 行）：每张卡 = 迷你界面缩略图（侧栏 + 内容条，用**它自己那套**配色）+ 中文名 + 气质标签 + 五色点 + 当前项 ✓。点一下就换、面板自动收起、写入 localStorage。**登录页右上角也放同一个组件**（登录页没有顶栏） |
...
- **v2.5 顶栏辨识度**：预览稿的 `--topbar-bg` 是 `rgba(255,255,255,.88)`，在浅色页面底上几乎与卡片同色（用户实测："顶部横栏跟其下背景一样是纯白，没有辨识度"）→ `main.css` 覆盖为「主题色 9%→3% 淡染渐变 + 2px `--border-strong` 分隔线 + 轻投影」，六套主题都成立。
...
| 5 | 全量回归：9 个产品机检 **507** 项（m0 13 / m1 15 / api-spec 24 / m2 17 / db-deep 9 / m3 36 / m3-http 142 / m4 30 / m4-http **221**）+ JUnit 白盒 **6 用例** + 预览稿自检 **211** 项 = **724 项** | 全部 0 失败（2026-09-23 晚实测，含课件 4.1/5.1 对齐后的复跑） |
| 6 | 本规格 §1~§9 与 `UI-PREVIEW.html`（**v8.3**）逐页对账 | 人工核对：路由、权限、四态、主题令牌；v8.2 另加 409 版本冲突提示与契约登记；v8.3 = M5 收口对账（下架入口 / 可见范围 / 字段名三处订正） |
