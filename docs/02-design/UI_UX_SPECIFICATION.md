...
| 6 | **换图 + 重裁**：师大蓝→时光塔高清版、青瓷→华砚湖畔、墨玉青→天下石牌坊；横幅输出宽 1000~1200px（原 760）；登录页背景按**显示框 1:1** 重裁（原先 0.9 比例被 cover 放大 1.15 倍 = 用户说的"蒙"），并给出两版候选 | 用户："原图挺好看的，裁剪质地后感觉又蒙又糊"；青瓷的花图看不出师大；墨玉青与师大蓝都是时光塔；师大蓝那张石碑"越看越丑"→ 改回时光塔高清版 | §3 |
...
**登录页背景（v7 修正）**：两版候选 —— **多花**（2000×1333，默认）与**时光塔广场**（5471×2914，预览稿可切换）。两版都按**登录左栏显示框的比例（≈1:1）裁成 1200×1200**；默认选多花，因为师大蓝横幅已经用了时光塔，重复使用会被用户当缺陷。
