...
| 版本 | **v2.5**（v2.4 之上做**用户上手实测后的四处返工**：顶栏辨识度、主题选择器独立占最右、登录页可换主题+塔图+右栏、工作台横幅按钮点击被装饰层挡住；v2.4 = M5 收口的**三处订正**：§8.5 布局的「发文单位→拟稿人」「可见范围改只读文本，并补积分标记」、§8.8 与 §10.6 的口径统一（下架无接口 ⇒ 无入口，预览稿同步删掉那个按钮）、§8.7「审核意见必填」按后端 `@NotBlank` 订正；v2.3 内容：§8.5 编辑页 **409 的两种处置**——状态冲突切只读 / 版本冲突不切只读并给"刷新内容"；v2.2 内容：顶栏弹层、行级悬停、A 版式与时光塔首页、主题三字名） |
| 日期 | 2026-09-23 |
| 状态 | **Frozen（已冻结）** |
...
| 5 | **v2.5 = 用户上手实测后的四处返工**（2026-09-24）：① 顶栏**要有辨识度**（原来与页面底几乎同色）—— 改主题色淡染玻璃 + 2px 分隔线 + 轻投影；② **主题选择器独立占顶栏最右**（原来夹在通知与身份之间），并加一道细分隔线；③ 登录页补主题选择器、塔图放大看见塔尖、右栏加主题底色与演示账号；④ 工作台横幅三个按钮**点不动**（装饰光晕挡指针，见 §8.2 根因） | 这四条都是"跑起来给人看"才暴露的：静态机检全绿、预览稿对照也一致，但**真实指针路径**与**窄/矮窗口**下的表现没人验过 → M6 走查清单要加"指针命中测试"与"两种窗口比例各截一张" | §8.1、§8.2、§8.15、`M5-CLOSURE §3 D11~D14` |
