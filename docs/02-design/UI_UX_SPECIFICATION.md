...
| 版本 | **v2.4**（v2.3 之上做 M5 收口的**三处订正**：§8.5 布局的「发文单位→拟稿人」「可见范围改只读文本，并补积分标记」、§8.8 与 §10.6 的口径统一（下架无接口 ⇒ 无入口，预览稿同步删掉那个按钮）、§8.7「审核意见必填」按后端 `@NotBlank` 订正；v2.3 内容：§8.5 编辑页 **409 的两种处置**——状态冲突切只读 / 版本冲突不切只读并给"刷新内容"；v2.2 内容：顶栏弹层、行级悬停、A 版式与时光塔首页、主题三字名） |
| 日期 | 2026-09-23 |
| 状态 | **Frozen（已冻结）** |
