...
| 版本 | **v2.3**（v2.2 的 v8 视觉定稿之上，补 §8.5 编辑页 **409 的两种处置**——状态冲突切只读 / 版本冲突不切只读并给"刷新内容"，对齐后端 `versionNum` 陈旧表单防覆盖；v2.2 内容：顶栏弹层、行级悬停、A 版式与时光塔首页、主题三字名） |
| 日期 | 2026-09-23 |
| 状态 | **Frozen（已冻结）** |
...
| 6 | 本规格 §1~§9 与 `UI-PREVIEW.html`（**v8.2**）逐页对账 | 人工核对：路由、权限、四态、主题令牌；v8.2 另加 409 版本冲突提示与契约登记 |
