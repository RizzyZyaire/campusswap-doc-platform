...
- **关键交互**：删除/彻底删除二次确认（彻底删除需**输入标题**确认，BR-08）；恢复失败 409 提示后刷新
  - **v2.6 新增入口**：页头加「上传 Markdown」—— 选本地 `.md` / `.markdown` / `.txt`，浏览器里读完直接 `POST /api/documents` 建草稿（标题取文件名、摘要取正文首个非标题段落），然后跳进编辑器补分类/标签。**只收纯文本**：Word/PDF 会明确提示"请先另存为 Markdown"（PRD 非目标 O7：不做 Office 在线预览与格式转换），超过 512 KB 也直接说明，不静默失败
