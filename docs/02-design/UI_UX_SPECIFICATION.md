...
| 版本 | **v2.1**（视觉复核：预览条/选择器位置、首页三版式、可读性；v2.0 的信息架构与主题体系不变） |
| 日期 | 2026-09-23 |
| 状态 | **Frozen（已冻结）** |
...
| 6 | **换图 + 重裁**：师大蓝→校训石碑、青瓷→华砚湖畔、墨玉青→天下石牌坊；横幅输出宽 1000~1200px（原 760） | 用户："原图挺好看的，裁剪质地后感觉又蒙又糊"；青瓷的花图看不出师大；墨玉青与师大蓝都是时光塔 | §3 |
...
| # | 规则 | 为什么 |
|---|---|---|
| V1 | **卡片描边 2px 且用主题色**（`--border`，悬停转 `color-mix(--accent 42%, --border)`） | 用户实测 1px 描边"太细了还是看得眼花"，板块之间的边界必须一眼分得清 |
| V2 | **卡片悬停整块上浮** `translateY(-2px)` + 阴影升到 `--shadow-lg` + 描边转主题色 | 用户："光标停留在哪块就放大"；上浮比缩放更稳（不会撞邻块） |
...
| V5 | 表格行悬停保留底色变化（`--surface-2`），与卡片悬停叠加时以卡片为准 | 表格是密集区域，上浮会晃眼，只换底色 |
...
**主题 ↔ 照片对应（v6，2026-09-23 第四轮反馈后重定）**：

| 主题 | 照片 | 源图（分辨率） | 裁剪 → 输出 | 说明 |
|---|---|---|---|---|
| `hebtu` 师大蓝 | **校训石碑** | 校训石碑.jpg（3601×2522） | 2400×912 → **1200×456** | 「怀天下 求真知」红字校训，全校最有辨识度的一帧；替换原来的时光塔（1000px 源图，放大后发糊） |
| `gingko` 银杏暖 | 校内行道 | 校内行道.jpg（1000×666） | 1000×380 → 1000×380 | 林荫道 + 暖色渐变 |
| `celadon` 青瓷 | **华砚湖畔** | 华砚湖畔.jpg（2000×1333） | 1520×578 → **1000×380** | 湖面 + 垂柳 + 连翘；**替换原「玉兰特写」**（用户："花图看不出师大"） |
| `ink` 墨玉青 | **天下石牌坊** | 天下.jpg（6000×4000） | 3600×1368 → **1200×456** | 石牌坊 + 深绿树，沉稳；**替换原「时光塔下」**（与师大蓝的时光塔重复） |
| `jiang` 师大绛 | 东门黎明 | 东门黎明.jpg（1000×465） | 1000×380 → 1000×380 | 黎明金红，呼应绛红 |
| `night` 墨夜 | 教学楼夜景 | 教学楼夜景.jpg（1000×563） | 1000×380 → 1000×380 | 夜景，呼应深色 |

> **裁剪纪律（v4 教训 + v6 加固）**：横幅照片一律按**显示比例**裁剪后再缩放，**输出宽度 ≤ 裁剪宽度（缩放比恒 < 1，脚本会直接抛错拒绝放大）**；
> 源图不足 1200px 的主题（校内行道 / 东门黎明 / 教学楼夜景）就输出 1000px，不许硬拉到 1200。
> 显示槽位：A 版式右侧卡片约 506px、B 版式右侧照片板约 663px、C 版式整条 1150×138——三种槽位都小于源图宽度，因此**任何版式都不放大**。
> 所有照片在文档里只内联一次，通过 `--photo-*` CSS 变量被横幅与画廊分别引用（自检断言：jpeg data URI 恰好 8 处）。
...
- **横幅三版式（v2.1）**：A 左文右图（信息密度最高，照片是右侧卡片）／**B 通栏大图 + 左侧文字浮层（M5 默认实现）**／C 细照片带 + 浅色信息卡（最清爽）。
  三版共用同一张主题照片，区别只在照片的呈现方式；**照片一律按高度贴合或按显示比例裁剪，缩放比恒 < 1**（不放大 = 不糊）。
  预览稿的版式切换器属于预览工具，不进入 M5 实现清单。
