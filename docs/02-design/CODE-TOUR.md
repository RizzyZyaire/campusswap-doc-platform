...
**欠账**：M5 前端未开工（`frontend/` 只有骨架目录）。M5 开工前**必须先做一次 UI 规格 v2 重写**：`docs/02-design/UI_UX_SPECIFICATION.md` 现有 8 条路由与「系统管理」聚合板块已按用户 2026-09-23 反馈作废，新口径见 `docs/02-design/UI-PREVIEW.html`（v2 预览稿：河北师大口径 / 5 套主题 / 五组导航 / 用户·角色与权限·组织机构独立成菜单 / 所有人可见「我的资料」）+ `docs/02-design/ui-preview.smoke.mjs`（124 项自检）。其余欠账：M6 测试与评审、M7 交付未做；Apifox 手动导入 + 发请求待你操作；防火墙 3306/6379 的入站放行规则未按建议收窄。
